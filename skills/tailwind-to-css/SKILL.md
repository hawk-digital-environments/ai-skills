...
## Source project Tailwind config (shadcn / custom themes)

When the source code uses a custom Tailwind theme (e.g. shadcn/ui with `bg-primary`, `text-muted-foreground`), the converter needs to know about those utilities or Tailwind's CLI will reject them.

**Step 1 — find the config**

```bash
node "${CLAUDE_SKILL_DIR}/scripts/find-closest-config.mjs" /path/to/source/file
# prints absolute path to tailwind.ConfigurationAspect.ts/js, or nothing
```

**Step 2 — write a stripped conversion config**

Read the source config and write only its `theme.extend` block to `work/tailwind.conversion.config.js` inside the skill's scripts directory. Strip `plugins`, `content`, `presets`, and any `require()` calls that reference packages not guaranteed to be installed.

```js
// work/tailwind.conversion.config.js  (generated by agent, not committed)
module.exports = {
    theme: {
        extend: {
            colors: {
                ...
                // … rest of colors from source config
            },
            borderRadius: {
                lg: 'var(--radius)',
...
The converter's `tailwind.config.js` detects this file automatically and merges the theme extensions before running — no extra CLI flags needed.

**Step 3 — map the resulting vars in the override file**

The custom utilities now resolve to CSS variable references (e.g. `background-color: hsl(var(--primary))`). Add entries to the `.tailwind-override.css` to redirect those to project tokens:
...
The converter deletes the work directory contents automatically after each run, so no cleanup is needed.
